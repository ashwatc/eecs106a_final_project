...
def skew_3d(omega):
    ...
    omega_hat[0, 1], omega_hat[0, 2] = -omega[2], omega[1]
    omega_hat[1, 0], omega_hat[1, 2] = omega[2], -omega[0]
    omega_hat[2, 0], omega_hat[2, 1] = -omega[1], omega[0]
    return omega_hat
...
def rotation_3d(omega, theta):
    ...
    rot = np.identity(3) + omega_hat / omega_norm * np.sin(omega_norm * theta) \
    + np.matmul(omega_hat, omega_hat) / omega_norm ** 2 * (1 - np.cos(omega_norm * theta))
    return rot
...
def homog_3d(xi, theta):
    ...
    if not np.any(xi[3:]):
    	...
    	g[0:3, 0:3] = rot
    	omega_norm = np.linalg.norm(omega)
    	g[0:3, 3] = 1 / omega_norm ** 2 * (np.matmul(np.identity(3) - rot, np.matmul(skew_3d(omega), v)) + omega * np.dot(omega, v) * theta)
    g[3, 3] = 1
    return g
