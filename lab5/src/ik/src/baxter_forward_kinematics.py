...
def baxter_forward_kinematics_from_angles(joint_angles):
    ...
    gst0[:3, 3] = qs[:, 7]
    gst0[3, 3] = 1
    vs = np.array([np.matmul(- kfs.skew_3d(w.T), q.T) for w, q in zip(ws.T, qs[:, :7].T)])
    xis = np.zeros((6, 7))
    xis[:3, :] = vs.T
    xis[3:, :] = ws
    g = np.matmul(kfs.prod_exp(xis, joint_angles), gst0)
    return g
...
def baxter_forward_kinematics_from_joint_state(joint_state):
    ...
    angles[:2] = joint_state.position[4:6]
    angles[2:4] = joint_state.position[2:4]
    angles[4:] = joint_state.position[6:9]
